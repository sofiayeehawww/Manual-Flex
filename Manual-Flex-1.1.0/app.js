...
(function () {
  ...
  const views = {
    ...
    // INSERCIÓN (Información/Glosario): dos vistas nuevas en el mismo
    // patrón mostrar/ocultar que ya usan home/library/detail.
  ...
  function parseRoute() {
    ...
    // INSERCIÓN (Información/Glosario): dos rutas nuevas.
  ...
  /* ---------------------------------------------------------------
     INSERCIÓN (Glosario) — Directorio de videos.
     Se genera en vivo a partir de BLOCKS/EXERCISES/GROUP_VIDEOS de
     data.js, así que nunca se puede "desincronizar": si mañana agregas
     un link nuevo en data.js, aparece aquí automáticamente sin tocar
     este archivo. Los ejercicios que comparten un mismo video (los
     bloques de grip/tobillo/circuito) se agrupan en una sola fila para
     no repetir el mismo enlace varias veces.
     --------------------------------------------------------------- */
  function renderVideoDirectory() {
    ...
    const blocksHTML = BLOCKS.map((b) => {
      ...
      // Agrupa por URL exacta para fusionar los links compartidos por bloque.
      const byUrl = new Map();
      const sinVideo = [];
      exs.forEach((ex) => {
        const url = exerciseVideoUrl(ex);
      ...
      // INSERCIÓN: antes, un bloque sin ningún video (ej. Bloque 9) se
      // omitía por completo de la lista. Ahora todo bloque aparece
      // siempre, aunque sea solo para dejar constancia de que el manual
      // original no trae video para esos ejercicios — así el directorio
      // cubre los 11 bloques sin huecos silenciosos.
  ...
  buildZoneChips();
  buildSecondaryChips();
  renderVideoDirectory(); // INSERCIÓN (Glosario) — una sola vez, es contenido estático
  renderRoute();
})();
