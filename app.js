/* =====================================================================
   APP.JS — Flexibilidad Activa
   Enrutamiento por hash, biblioteca de ejercicios (filtros y tarjetas),
   vista de detalle, glosario y visor de imágenes (lightbox).
   ===================================================================== */
(function () {
  "use strict";
  ...
  /* =====================================================================
     SECCIÓN — ICONOGRAFÍA
     Set de iconos de línea (24×24, stroke=currentColor). Los iconos de
     zona corporal se reutilizan en los chips de filtro, las tarjetas de
     la biblioteca y la vista de detalle.
     ===================================================================== */
  const ICONS = {
  ...
  // Iconos de zona corporal (hombro, tórax, antebrazo, columna, cadera,
  // rodilla, tobillo, pie) — glifos de línea legibles entre 14 y 34px.
  const ZONE_ICONS = {
  ...
  /* =====================================================================
     SECCIÓN — ESTADO Y FUNCIONES AUXILIARES
     Estado de filtros de la biblioteca y helpers de lectura de data.js.
     ===================================================================== */
  ...
  /* =====================================================================
     ...
     Enrutamiento por hash entre las cinco vistas de la aplicación:
     #/               → Inicio
  ...
  function parseRoute() {
    ...
    if (raw === "/informacion") return { name: "info" };
    if (raw === "/glosario") return { name: "glossary" };
    return { name: "home" };
  }
  ...
  function setActiveTabIndicator(routeName) {
    const targetRoute = ROUTE_TO_TAB[routeName] || "/";
  ...
  /* =====================================================================
     SECCIÓN — VISTA BIBLIOTECA
     Chips de filtro (zona, bloque, equipo), buscador y grid de tarjetas.
     ===================================================================== */
  ...
  /* =====================================================================
     SECCIÓN — VISTA DETALLE
     Ficha completa de un ejercicio: galería de medios, video de
     referencia, enfoque/objetivo, dosificación, técnica y nota anatómica.
     ===================================================================== */
  ...
  /* =====================================================================
     SECCIÓN — VISTA GLOSARIO: DIRECTORIO DE VIDEOS
     Genera, agrupado por bloque, el listado de videos de referencia a
     partir de BLOCKS/EXERCISES/GROUP_VIDEOS. Los ejercicios que
     comparten un mismo video (grip, tobillo, circuito) se fusionan en
     una sola fila.
     ===================================================================== */
  function renderVideoDirectory() {
    const container = document.getElementById("videoDirectory");
    if (!container) return;

    const blocksHTML = BLOCKS.map((b) => {
      const exs = EXERCISES.filter((e) => e.block === b.id);

      // Agrupa por URL exacta para fusionar los enlaces compartidos por bloque.
      const byUrl = new Map();
      const sinVideo = [];
      exs.forEach((ex) => {
        const url = exerciseVideoUrl(ex);
        if (!url) { sinVideo.push(ex); return; }
        if (!byUrl.has(url)) byUrl.set(url, []);
        byUrl.get(url).push(ex);
      });

      // Todo bloque se muestra siempre, incluso sin ningún video: en
      // ese caso solo se deja constancia de qué ejercicios no lo tienen.
      const missingNote = sinVideo.length
        ? `<p class="video-block-missing">Sin video en el manual original: ${sinVideo.map((e) => escapeHtml(e.name)).join(", ")}.</p>`
        : "";

      if (byUrl.size === 0) {
        return `
          <div class="video-block">
            <h3 class="video-block-title">Bloque ${b.id} · ${escapeHtml(b.title)}</h3>
            ${missingNote}
          </div>`;
      }

      const rows = [...byUrl.entries()].map(([url, group]) => `
        <a class="video-row" href="${url}" target="_blank" rel="noopener">
          ${ICONS.play}
          <span class="video-row-text">
            ${group.length > 1 ? `<span class="video-row-note">Un mismo video cubre ${group.length} ejercicios</span>` : ""}
            <span class="video-row-names">
              ${group.map((e) => `<span class="video-name-chip">${escapeHtml(e.name)}</span>`).join("")}
            </span>
          </span>
          <span class="video-row-go">Ver ↗</span>
        </a>`).join("");
  ...
  /* =====================================================================
     SECCIÓN — LIGHTBOX
     Visor de imágenes a pantalla completa para la galería de medios.
     ===================================================================== */
